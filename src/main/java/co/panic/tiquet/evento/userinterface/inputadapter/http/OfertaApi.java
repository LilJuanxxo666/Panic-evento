...
public class OfertaApi {
    ...
    public ResponseEntity<Oferta> consultarOfertaPorId(@PathVariable UUID id) {
        Oferta ofertaConsultada = ofertaUseCase.listarPorID(id);
        return new ResponseEntity<>(ofertaConsultada, HttpStatus.OK);
    }

    @GetMapping
    public ResponseEntity<List<Oferta>> consultarOfertas() {
        List<Oferta> listaOfertas = ofertaUseCase.listarTodo();
